                    ...
                    .end(function(err, res){
                        ...
                        if(err){
                            ...
                            if (typeof res.body[0] === "undefined" || res.body[0] === null) {
                                message_out = "Your email and/or password are incorrect." + '<br>' + "If you don't have an account, create one in the ACCOUNT tab.";
                                document.getElementById("msg").innerHTML = message_out;
                            } else {
...
function apply_operation(table_name,email,amount,balance,rec_type){
    ...
    var currentDate = new Date();
    var rec_stampUTC = currentDate.toISOString();
    var id_track = (document.getElementById('userlogin').innerHTML).substring(18);
...
//**-------------------- INSERT COLUMN OF TYPE OF OPERATION --------------------**//
                                        if (i==0){
                                            var text = document.createTextNode('Applied by...');
                                        } else {
                                            var text = document.createTextNode(res.body[i].id_track);
                                        }
